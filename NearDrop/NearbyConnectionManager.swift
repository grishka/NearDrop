...
class NearbyConnectionManager : NSObject, NetServiceDelegate, InboundNearbyConnectionDelegate, UNUserNotificationCenterDelegate{
    ...
    func obtainUserConsent(for transfer: TransferMetadata, from device: RemoteDeviceInfo, connection: InboundNearbyConnection) {
        ...
        if isSavedDevice(name: device.name) {
            ...
            notificationContent.subtitle = String(format: NSLocalizedString("DeviceSendingFiles", value: "%1$@ is sending you %2$@", comment: ""), arguments: [device.name, fileStr])
            ...
            acceptActiveConnection(transferId: connection.id, accepted: true)
            return;
        }
        let notificationContent=UNMutableNotificationContent()
        notificationContent.title="NearDrop"
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
        ...
        acceptActiveConnection(
            transferId: response.notification.request.content.userInfo["transferID"]! as! String,
            accepted: accepted
        )
        completionHandler()
    }
    
    func alwaysAcceptSafetyCheck() -> AlwaysAcceptResponse {
        let alert = NSAlert()
        alert.messageText = NSLocalizedString("AlwaysAcceptWarning", value: "Warning", comment: "")
        alert.alertStyle = .warning
        alert.addButton(withTitle: NSLocalizedString("Proceed", value: "Proceed", comment: ""))
        alert.addButton(withTitle: NSLocalizedString("AcceptOnce", value: "Accept Once", comment: ""))
    ...
    func acceptActiveConnection(transferId: String, accepted: Bool) {
